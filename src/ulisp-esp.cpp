...
// Added for PlatformIO
// frm https://eightytwo.net/blog/running-ulisp-on-an-esp32/
...
// danny added
// click the Problem, right-click the call -> go to definition
...
bool eqsymbols(object *obj, char *buffer)
{
  ...
  while (!(arg == NULL && buffer[i] == 0))
  {
    if (arg == NULL || buffer[i] == 0 ||
        arg->chars != (buffer[i] << 24 | buffer[i + 1] << 16 | buffer[i + 2] << 8 | buffer[i + 3]))
      return false;
    arg = car(arg);
    i = i + 4;
  }
...
void pslice(object *array, int size, int slice, object *dims, pfun_t pfun, bool bitp)
{
  ...
  for (int i = 0; i < d; i++)
  {
    ...
    {
      if (bitp)
        pint(((*arrayref(array, index >> 5, size))->integer) >> (index & 0x1f) & 1, pfun);
      else
...
object *sp_setf(object *args, object *env)
{
  ...
  while (args != NULL)
  {
    ...
    if (bit == -1)
      ...
      *loc = number((checkinteger(SETF, *loc) & ~(1 << bit)) | checkbitvalue(SETF, arg) << bit);
    args = cddr(args);
  }
  return arg;
}
...
object *sp_withsdcard(object *args, object *env)
{
  ...
  if (mode >= 1)
  {
    ...
    if (!SDpfile)
      error2(WITHSDCARD, PSTR("problem writing to SD card or invalid filename"));
  }
  else
  {
    char buffer[BUFFERSIZE];
    ...
    if (!SDgfile)
      error2(WITHSDCARD, PSTR("problem reading from SD card or invalid filename"));
  }
...
object *sp_withclient(object *args, object *env)
{
  ...
  else
  {
    ...
    int success;
    if (stringp(address))
      success = client.connect(cstring(address, buffer, BUFFERSIZE), checkinteger(WITHCLIENT, port));
    else if (integerp(address))
      success = client.connect(address->integer, checkinteger(WITHCLIENT, port));
    else
...
object *fn_ceiling(object *args, object *env)
{
  ...
  args = cdr(args);
  if (args != NULL)
    return number(ceil(checkintfloat(CEILING, arg) / checkintfloat(CEILING, first(args))));
  else
...
object *fn_floor(object *args, object *env)
{
  ...
  args = cdr(args);
  if (args != NULL)
    return number(floor(checkintfloat(FLOOR, arg) / checkintfloat(FLOOR, first(args))));
  else
...
object *fn_truncate(object *args, object *env)
{
  ...
  args = cdr(args);
  if (args != NULL)
    return number((int)(checkintfloat(TRUNCATE, arg) / checkintfloat(TRUNCATE, first(args))));
  else
...
object *fn_round(object *args, object *env)
{
  ...
  args = cdr(args);
  if (args != NULL)
    return number(myround(checkintfloat(ROUND, arg) / checkintfloat(ROUND, first(args))));
  else
...
object *fn_settextcolor(object *args, object *env)
{
  ...
  (void)env;
  if (cdr(args) != NULL)
    tft.setTextColor(checkinteger(SETTEXTCOLOR, first(args)), checkinteger(SETTEXTCOLOR, second(args)));
  else
...
EVAL:
  ...
  if (symbolp(function))
  {
    ...
    if (((name > 0) && (name < SPECIAL_FORMS)) || ((name > KEYWORDS) && (name < USERFUNCTIONS)))
